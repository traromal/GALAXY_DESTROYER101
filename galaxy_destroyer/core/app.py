...
from .render import (
    ...
    style, BorderStyle, DOUBLE_BORDER, word_wrap
)
        ...
        try:
            if select.select([sys.stdin], [], [], 0)[0]:
                ...
                if char == '\x1b':
                    ...
                    ctrl = char in '\x00\x01\x02\x03\x04\x05\x06\x07\x08\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f'
...
class GalaxyApp:
    ...
    def _render_output(self, width: int, height: int):
        ...
        if len(self.state.output_buffer) > output_height:
            write_line(style(f"  ↑ {len(self.state.output_buffer) - output_height} more lines", color="gray"))
    ...
    def _render_input(self, width: int):
        ...
        if self.state.cursor_pos < len(self.state.input_buffer):
            move_cursor(len(prompt) + self.state.cursor_pos + 1, 
                       get_terminal_size()[1] - 2)
